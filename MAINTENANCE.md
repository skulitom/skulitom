...
Run `python scripts/generate_link_buttons.py` to rebuild the compact web-app buttons and main portfolio button in `assets/links/v1/`. Keep each button as its own linked image, so they wrap naturally on narrower screens. The graphics are original, static SVG with no external dependencies. Use a new version directory if changing published artwork.
